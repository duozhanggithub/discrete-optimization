...
def scip_solver_3(customers, customer_count, vehicle_count, vehicle_capacity):
    ...
    for v in Vr:
        model.addCons(D[0,v] == Q, "c5(%s)" % v)
        for i in Am:
            model.addCons(D[i,v] <= Q, "c4(%s,%s)" % (i,v))
            #if i > 0:
            model.addCons(D[i,v] >= 0, "c8(%s,%s)" % (i,v))
        model.addCons(quicksum(X[v,0,j] for j in Am) <= 1, "c2(%s)" % v)
        model.addCons(quicksum(X[v,i,j]*(d[i]+d[j]) for i in Am for j in Am) <= Q, "c6(%s)" % (v))

    for j in A:
        if j > 0:
            model.addCons(quicksum(X[v,i,j] for v in Vr for i in A) == 1, "c1(%s)" % (j))
        for v in Vr:
            model.addCons(quicksum(X[v,i,j] for i in A) - \
                quicksum(X[v,j,i] for i in A) == 0, "c3(%s,%s)" % (j,v))

    for i in Am:
        for j in Am:
            for v in Vr:
                model.addCons((D[i,v] - d[j] - D[j,v])*X[v,i,j] == 0, "c7(%s,%s,%s)" % (i,j,v))
